    ...
    if (testData != NULL) {
        if (testData->ref != NULL) {
            ...
            *newRef = testData->threadIndex;
            ...
            LOG_DEBUG("Old Atomic Ref = %x", testData->ref);
            bool success = cmpAndSet(&(testData->ref), oldRef, newRef, oldStamp, newStamp, testData->threadIndex);
            if (success) {
                testData->cfg->threadSucessStatus[testData->threadIndex] = THREAD_CAS_SUCCESS;
                LOG_DEBUG("New Atomic Ref = %x", testData->ref);
...
bool doTestAtomicStampedRef(TestConfigAtomicStampedRef* cfg) {
    LOG_PROLOG();
...
int testAtomicStampedRefmain() {
    LOG_INIT_CONSOLE();
	LOG_INIT_FILE();

    #define COUNT_TEST_CONFIG 1
